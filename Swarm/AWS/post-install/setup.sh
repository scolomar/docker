function send_list_command {
 local command="$1" ;
 local target="$2" ;
 local CommandId=$( aws ssm send-command --document-name "AWS-RunShellScript" --parameters commands="$command" --targets Key=tag:"aws:cloudformation:logical-id",Values="$target" --query "Command.CommandId" --output text ) ;
 while true ; do
...
for target in $targets ; do
 send_list_command "$command" "$target" ;
done ;
